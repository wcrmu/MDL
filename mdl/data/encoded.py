...
def _feature_specs(manifest: dict[str, Any]) -> list[dict[str, Any]]:
    tokenization = manifest.get("tokenization")
    if not isinstance(tokenization, dict):
        raise ValueError("manifest must contain tokenization")
    return list(tokenization.get("features", []))
...
def _collate_sequence(values: list[list[Any]], source: dict[str, Any]) -> dict[str, Tensor]:
    ...
    padded = [
        value + [padding_value] * (max_length - len(value))
        for value in values
    ]
    return {
...
class EncodedTabularDataset(IterableDataset[dict[str, Any]]):
    ...
    def __iter__(self) -> Iterator[dict[str, Any]]:
        ...
        feature_specs = _feature_specs(self.manifest)
