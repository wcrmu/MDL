...
上报 AUC 用线上排序评估器的估计量：`auc_bins` 是固定概率阈值的个数（阈值网格
`[-eps, 1/(N-1), ..., 1, 1+eps]`），每个阈值累一份 TP/FP/TN/FN，再对得到的
(FPR, TPR) 序列做梯形积分。这样每个 rank 只需 all-reduce 一个 `(4, N)` 的混淆
矩阵。它不是 `sklearn.roc_auc_score` 的 Mann-Whitney U 精确值——等价说法是：
分箱后的 Mann-Whitney（并列对按 0.5 计）和同一分箱下的梯形积分逐位相同。

`N` 取 4096 而不是线上的 3000。估计量对分辨率单调：网格更细只会让结果更靠近
精确 AUC，正常任务两者差 ~1e-4，预测压成窄带时 3000 会多丢 0.04 量级。

固定概率网格的分辨率取决于预测的概率跨度，而不是 base rate：只要 logit 分布
正常，0.3% 正样本率的 `upid_pay` 在 4096 箱下也只差 ~2e-4；只有预测真的压进
极窄带才会显著偏低。fixed-test 因此并行跑第二个 log-odds 轴（`[-20, 20]`，
4096 箱）的诊断 AUC，两者差超过 0.005 时打 `auc_grid_drift` 警告，并带上占用
的阈值数——这是区分「排序确实差」和「预测压在一个窄带里」的依据。诊断值不上报
为主指标。
