...
如果要让 agent 在内部机器上实现 feature pipeline，请使用 [feature_pipeline_agent_playbook.md](feature_pipeline_agent_playbook.md) 中的 prompt、阶段拆分和验收标准。特征工程设计和检查请同时使用 [feature_engineering_checklist.md](feature_engineering_checklist.md)。
...
推荐把 feature pipeline 放在 `MDL` 同级目录：
...
```text
<workspace>/MDL_feature_pipelines/
  your_dataset/
    README.md
    ...
    raw/                         # optional local raw data, usually not committed
      .gitkeep
    processed/                   # feature pipeline output, usually not committed
      .gitkeep
    src/
      __init__.py
...
```bash
cd <project-root>
python scripts/preprocess.py --data-dir ../MDL_feature_pipelines/your_dataset/processed
python scripts/train.py --data-dir ../MDL_feature_pipelines/your_dataset/processed --epochs 1 --batch-size 32 --max-steps 2 --eval-max-batches 2
```
...
```bash
cd ../MDL_feature_pipelines/your_dataset
python preprocess.py --raw-path raw/input.csv --output-dir processed

cd <project-root>
python scripts/preprocess.py --data-dir ../MDL_feature_pipelines/your_dataset/processed
python scripts/train.py --data-dir ../MDL_feature_pipelines/your_dataset/processed --epochs 1 --max-steps 2
```
