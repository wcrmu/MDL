...
- 不修改 `MDL/` 仓库中的任何文件。
- 只在仓库内 feature pipeline 目录中创建和修改文件，例如 `<project-root>/data/pipelines/<dataset_name>/`。
- 不把 raw 数据复制进 `MDL/` 仓库。
- 不把 raw 大文件提交到 git。
...
最终目标：
在 <project-root>/data/processed/<dataset-name> 下生成 manifest.json、train.csv、val.csv、test.csv，并且从 <project-root> 运行以下命令通过：

python scripts/preprocess.py --data-dir data/processed/<dataset-name>
python scripts/train.py --data-dir data/processed/<dataset-name> --epochs 1 --batch-size 32 --max-steps 2 --eval-max-batches 2
```
...
<project-root>/
  data/
    ...
    processed/
      <dataset-name>/
    raw/
      <dataset-name>/      # optional symlink or local ignored directory
    fixtures/
      <dataset-name>/
```
...
<project-root>/data/processed/<dataset-name>/*
...
- data/processed/<dataset-name>/manifest.json 存在。
- data/processed/<dataset-name>/train.csv 存在。
...
python scripts/preprocess.py --raw-path <raw-data-path> --output-dir <project-root>/data/processed/<dataset-name>
...
最后一条 `git status --short` 不应该出现 feature pipeline 之外的 MDL core 修改。
