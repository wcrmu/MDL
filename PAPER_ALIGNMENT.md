...
This repository keeps four model surfaces aligned with the local paper sources:

- `rankmixer`: Pure RankMixer-style feature-token baseline without MDL scenario/task/domain modules.
...
- `rankmixer` and `mdl_rankmixer` default to `tokenization.feature_tokenizer: rankmixer` with 32 feature tokens of width 768; `rankmixer` has no scenario/task tokens, while `mdl_rankmixer` keeps them outside the feature-token count. Encoded inputs are concatenated in YAML order and must exactly equal `num_feature_tokens * token_dim`; implicit zero padding is disabled before the direct reshape. The default template now uses per-sequence multi-slice summaries plus a small explicit context dense slice rather than a large packed placeholder.
- `auto_split` remains available only as an explicit OneTrans-style engineering fallback.
- Multi-field behavior sequences are first fused per step. `rankmixer` and `mdl_rankmixer` consume a LONGER-style sequence encoder with target/global tokens, recent query compression, Token Merge, InnerTrans, time-delta side fields fused in the step MLP, cross-causal attention, self-causal refinement, and configurable multi-slice RankMixer summaries.
