#!/usr/bin/env python3
"""Build or refresh production model YAMLs from sample fields and optional stats.
...
TASK_IMPORTANT_FIELDS_BY_TASK = {
    ...
    "upid_pay": (
        ...
        # collision the way the identity tables do.
        "idx_c_ordr_cnt_15d_hn",
        "nfk_gmv_14d_hn",
        # Always-present user-side purchase propensity.  buy_long is empty for
        # ~24% of requests, so the pay prior contributes nothing there; this is
...
def task_important_name(task: str, source: str) -> str:
    """Per-task important table name.

    Task-important tables used to be one shared table per source, so a source
    listed by several tasks was optimised mostly by whichever task carried the
    largest loss weight.  With fst_cart at 0.5 and the two tail tasks at 0.01
    that left ~85% of the upid_pay domain prompt riding on cart-driven
    embeddings.  Each task now owns its copy; apart from the identity tables
    (which are no longer shared by construction) these are small.
    """

    return f"task_{task}_important_{source}"


def _is_task_important_table(table_name: str) -> bool:
    return table_name.startswith("task_") and "_important_" in table_name
...
TASK_SUPPLEMENTAL_PRIOR_FIELD_ALLOWLIST = {
    "upid_pay": frozenset(
        {
            "time",
            "cat1_id_hn",
...
TASK_SUPPLEMENTAL_PRIOR_NULL_ANCHOR = {
    "upid_pay": "cat_id_hn",
}
...
def task_prior_inputs(task: str) -> list[str]:
    """Logical prior sequence names attached to one task token."""

    names = [f"task_{task}_prior"]
    if task in TASK_SUPPLEMENTAL_PRIOR_UPS:
        ups = TASK_SUPPLEMENTAL_PRIOR_UPS[task]
        names.append(f"task_{task}_{ups}_prior")
    return names
...
_BOOTSTRAP_PROFILE_DRIVEN_EMBEDDING_SHAPES: dict[str, tuple[int, int]] = {
    "ad_id_bin_hn": (256, 8),
    "adj_cartcvr_hn": (1024, 16),
    "adj_ctr_hn": (1024, 16),
    "adj_cvr_hn": (1024, 16),
    "auto_price_p05_dis": (4096, 16),
    "auto_price_p10_dis_hn": (2048, 16),
    "auto_sales_p10_dis": (2048, 16),
    "buy_long_spec_vids_hn": (33554432, 32),
    "buy_long_x_cat1_id_hn": (1024, 16),
    "buy_long_x_cat2_id_hn": (16384, 24),
    ...
    "buy_long_x_price_hn": (1024, 16),
    "buy_long_x_sales_hn": (1024, 16),
    "buy_long_x_sku_ids_hn": (268435456, 32),
    "buy_long_x_spec_hn": (33554432, 32),
    "buy_long_x_timegap_hn": (512, 16),
    "campaign_id_hn": (8388608, 32),
    "cart_7d_cat1_ids_hn": (1024, 16),
    "cart_cnt_1d_hn": (1024, 16),
    "cart_cnt_3d_hn": (1024, 16),
    "cart_hit_i2i_idx_hn": (1024, 16),
    "cart_long_hit_samestyle_i2i_idx_hn": (1024, 16),
    "cart_long_spec_vids_hn": (67108864, 32),
    "cart_long_x_cat1_id_hn": (1024, 16),
    "cart_long_x_cat2_id_hn": (16384, 24),
    ...
    "cart_long_x_price_hn": (1024, 16),
    "cart_long_x_sku_ids_hn": (536870912, 32),
    "cart_long_x_spec_hn": (67108864, 32),
    "cart_long_x_timegap_hn": (512, 16),
    "cat1_id_hn": (1024, 16),
    "cat2_id_hn": (16384, 24),
    "cat3_id_hn": (16384, 32),
    "cat4_id_hn": (65536, 32),
    "cat_id_hn": (131072, 32),
    "clk_1d_cat_cnt_hn": (4096, 16),
    "clk_3d_cnt_hn": (2048, 16),
    "clk_7d_page_sns_hn": (16384, 24),
    "clk_cnt_1d_hn": (1024, 16),
    "clk_hit_i2i_idx_hn": (1024, 16),
    "clk_long_goods_abs_timegap_1d_hn": (256, 8),
    "clk_long_x_cat1_id_hn": (1024, 16),
    "clk_long_x_cat2_id_hn": (16384, 24),
    ...
    "clk_long_x_price_hn": (1024, 16),
    "clk_long_x_sales_hn": (1024, 16),
    "clk_long_x_timegap_hn": (512, 16),
    "create_time_hn": (256, 8),
    "currency_hn": (1024, 16),
    "f_goods_view_times_tg_l1_hn": (16384, 24),
    "flatten_query_hash_x_flat_q_hash_hn": (2097152, 32),
    "flatten_query_hash_x_timegap_hn": (512, 8),
    ...
    "goods_avlb_sku_num_dis_hn": (1024, 16),
    "goods_cluster_id_1w_hn": (32768, 32),
    "goods_id_hn": (268435456, 32),
    "goods_name_bigram_hn": (33554432, 32),
    "goods_ner_infos_hn": (4194304, 32),
    "goods_onsale_sku_num_dis_hn": (1024, 16),
    "goods_query_emb32v3_cos_hn": (4096, 16),
    "goods_scene_clk_cnt_15d_hn": (4096, 16),
    "goods_title_tfidf_term_hash_list_hn": (524288, 48),
    "hash_language_site_hn": (2048, 16),
    "i2i2cat2_swing_hn": (16384, 24),
    "i2i_coclk_hn_share": (33554432, 32),
    "i2i_hit_site_q2i_idx_hn": (2048, 16),
    ...
    "idx_c_adj_cart_cvr_15d_hn": (1024, 16),
    "idx_c_adj_ctr_15d_hn": (1024, 16),
    "idx_c_adj_ordr_cvr_15d_hn": (1024, 16),
    "idx_c_cart_cnt_15d_hn": (1024, 16),
    "idx_c_clk_cnt_15d_hn": (1024, 16),
    "idx_c_impr_cnt_15d_hn": (1024, 16),
    "idx_c_ordr_cnt_15d_hn": (1024, 16),
    "idx_goods_creative_id_hn": (33554432, 32),
    "impr_3h_tg_hn": (512, 8),
    "impr_all_tg_hn": (512, 16),
    "impr_cat_clk_goods_ids_cnt_1d_hn": (4096, 24),
    "impr_clk_6h_cnt_hn": (1024, 24),
    "impr_long_goods_abs_timegap_1d_hn": (256, 8),
    "impr_x_cat1_id_hn": (1024, 16),
    "impr_x_cat2_id_hn": (16384, 24),
    ...
    "impr_x_page_sn_hn": (16384, 24),
    "impr_x_price_hn": (1024, 16),
    "impr_x_sales_hn": (1024, 16),
    "impr_x_timegap_hn": (512, 8),
    "is_promotion_hn": (256, 8),
    "language_hn": (512, 16),
    "list_clk_cat1_ids_hn": (1024, 16),
    "list_clk_cat_ids_hn": (65536, 32),
    "main_goods_ids_hn_share": (16777216, 32),
    "mall_id_hn": (4194304, 32),
    "mid_cmprc_diff_list_dis": (2048, 16),
    "mid_goods_prc_list_dis": (4096, 16),
    "mkt_prc_hn": (1024, 16),
    "multimodal_i2i_hit_cart_size_hn": (4096, 16),
    "multimodal_i2i_hit_clk_size_hn": (2048, 16),
    "nfk_gmv_14d_hn": (1024, 16),
    "nfk_price_14d_hn": (1024, 16),
    "nfk_sales_14d_hn": (1024, 16),
    "offline_outside_goods_id_list_hn_share": (33554432, 32),
    "only_semi_swingi2i_cut60_hn_share": (8388608, 32),
    "opt_id_hn": (4096, 32),
    "ori_price_hn_share": (1024, 16),
    "origin_query_hash_hn": (1048576, 32),
    "page_elsn_hn": (4096, 16),
    "page_sn_hn": (4096, 24),
    "plat_hn": (256, 8),
    "price_after_promotion_div_hn": (8192, 32),
    "price_after_promotion_hn": (1024, 16),
    "price_bef_coupon_hn": (1024, 16),
    "price_hn": (1024, 16),
    "promotion_discount_hn": (4096, 16),
    ...
    "query_pay_cnt_15d_hn": (2048, 16),
    "query_terms_hash_hn": (131072, 48),
    "query_tfidf_term_hash_list_hn": (262144, 48),
    "recall_merge_cate1_ids_hn": (1024, 16),
    "recall_merge_cate_ids_hn": (131072, 32),
    "recall_merge_cate_levels_hn": (256, 8),
    "region_hn": (1024, 16),
    "rel_level_hn": (256, 8),
    "rel_score_hn": (1024, 16),
    "rev_ratings_cnt_crs_pos_hn": (16384, 24),
    "sales_hn": (1024, 16),
    "scene_adj_cartcvr_15d_hn": (4096, 16),
    "scene_adj_ctr_15d_hn": (4096, 16),
    "scene_adj_cvr_15d_hn": (4096, 16),
    "scene_cart_cnt_15d_hn": (4096, 16),
    "scene_id_hn": (4096, 16),
    "scene_impr_cnt_15d_hit_hn": (1024, 16),
    "scene_impr_cnt_15d_hn": (32768, 32),
    "search_method_hn": (1024, 16),
    "sellr_type_hn": (256, 8),
    "semi_clk_x_cat_id_hn": (65536, 32),
    "semi_clk_x_goods_id_hn": (8388608, 32),
    "semi_clk_x_mall_id_hn": (262144, 48),
    "semi_clk_x_page_sn_hn": (32768, 24),
    "semi_clk_x_timegap_hn": (512, 8),
    "semi_swingi2i_cut30_hn_share": (4194304, 32),
    "sess_q2q_hash_list_hn": (8388608, 32),
    "show_price_hn": (1024, 16),
    "site_id_hn": (1024, 16),
    "site_q2i_good_list_hn_share": (8388608, 32),
    "site_x_asian_code_hn": (4096, 16),
    "sku_cart_cnt_7d_hn": (4096, 16),
    "sku_id_hn": (134217728, 32),
    "sku_ordr_cnt_1m_hn": (4096, 16),
    "sku_price_dis_hn": (2048, 16),
    "sku_price_v2_hn": (4096, 16),
    "sku_sales_dis_hn": (1024, 16),
    "sku_sales_hn": (1024, 16),
    "sku_spec_hash_hn": (16777216, 32),
    "sku_spec_hn": (16777216, 32),
    "sku_spec_vids_hn": (16777216, 32),
    "srch_q2i_x_cat1_id_hn": (1024, 16),
    "srch_q2i_x_cat2_id_hn": (16384, 24),
    ...
    "srch_q2i_x_timegap_hn": (512, 16),
    "target_gs_last_cart_tg_hn": (512, 16),
    "timezone_hn": (32768, 24),
    "tit_in_top_query_cnt_hn": (8192, 24),
    "u_fst_ordr_cnt_mix_d_hn": (8192, 16),
    "uid_or_bg_hn": (16777216, 32),
    "ups_clk_sku_x_cat1_id_hn": (1024, 16),
    "ups_clk_sku_x_cat2_id_hn": (16384, 24),
    ...
    "ups_clk_sku_x_timegap_hn": (512, 16),
    "ups_clkv2_i2i_goods_ids_hit_all_size": (1024, 16),
    "ups_clkv2_i2i_goods_ids_hit_size": (1024, 16),
    "ups_in_cart_2h_sku_cur_prices_hn": (256, 8),
    "ups_in_cart_goods_hn_share": (67108864, 32),
    "ups_in_cart_tg_hn": (512, 16),
    "ups_incart_cat1_id_nc_hn": (4096, 24),
    "ups_query_term_hash_v2_hn": (2097152, 32),
    "ups_query_tg_hn": (512, 8),
    "ups_search_method_hash_hn": (1024, 16),
    "us_ctr_price_dis50_hn": (2048, 16),
    "view_30m_cat1_ids_hn": (1024, 16),
    "view_7d_page_elsns_hn": (8192, 16),
    "view_7d_page_sns_hn": (16384, 24),
    "view_long_x_cat1_id_hn": (1024, 16),
    "view_long_x_cat2_id_hn": (16384, 24),
...
def validate_profile_report(
    ...
    # Empty map means "not measured" (e.g. profiler read the wrong label path
    # or --estimate-from-names). Only a non-empty map is treated as a claim that
    # every expected task has a binary label breakdown.
    if isinstance(label_distribution, Mapping) and label_distribution:
        ...
        # null/-1 categories were added, and for empty/missing label_distribution.
        # Those reports can prove only that every label was binary, so any
        # aggregate invalid count remains fatal.
...
def _task_prior_sequence(
    ...
    *,
    ...
    null_anchor_field: str = "goods_id_hn",
) -> dict[str, Any]:
    ...
    for raw_field in source_sequence.get("fields", []):
        field_name = str(raw_field["name"])
        source = str(raw_field["source"])
        is_time = field_name == "time" or source.endswith("_x_time")
        if field_allowlist is not None:
            allow_key = "time" if is_time else field_name
            if allow_key not in field_allowlist:
                continue
        if is_time:
            fields.append(
                {
    ...
    return {
        "name": sequence_name or f"task_{task}_prior",
        ...
        "time_delta_field": TIME_DELTA_FIELD,
        "null_anchor_field": null_anchor_field,
        "fields": fields,
    }
...
def _apply_profile_driven_embedding_shapes(payload: dict[str, Any]) -> None:
    ...
    for table_name, entry in entries.items():
        ...
        elif _is_task_important_table(table_name):
...
def build_config(
    ...
    *,
    ...
    task_important_by_task: dict[str, dict[str, str]] = {
        task: {} for task in TASK_IMPORTANT_FIELDS_BY_TASK
    }
    ...
    if mdl_family:
        ...
        for task, sources in TASK_IMPORTANT_FIELDS_BY_TASK.items():
            for source in sources:
                name = task_important_name(task, source)
                features.append(_independent_feature(name, source, "task", values))
                task_important_by_task[task][source] = name
    ...
    task_prior_sequences = (
        [
            _task_prior_sequence(
                ...
                max_sequence_length=max_sequence_length,
                target_inputs=tuple(
                    task_important_by_task[task][source]
    ...
    if mdl_family:
        for task, ups in TASK_SUPPLEMENTAL_PRIOR_UPS.items():
            task_prior_sequences.append(
                _task_prior_sequence(
                    task,
                    sequence_by_name[ups],
                    values,
                    ...
                    sequence_name=f"task_{task}_{ups}_prior",
                    field_allowlist=TASK_SUPPLEMENTAL_PRIOR_FIELD_ALLOWLIST[task],
                    null_anchor_field=TASK_SUPPLEMENTAL_PRIOR_NULL_ANCHOR[task],
                )
            )
    sequences = [
        *main_sequences,
        ...
        [
            {
                ...
                "prior_inputs": task_prior_inputs(task),
            }
            for task in EXPECTED_LABELS
        ]
        if mdl_family
    ...
    payload: dict[str, Any] = {
        ...
        "training": {
            ...
            "fixed_test_eval": {
                ...
                "files_per_rank": 4,
                # Same fixed-threshold estimator as the online ranking
                # evaluator, on a finer grid than the 3000 it uses.
                "auc_bins": 4096,
            },
            ...
            "checkpoint": {
                ...
                # not grow with the requested memory, which is far below the
                # concurrent per-node staging need. /dev/shm is sized by the
                # launch request.
...
def render_config(payload: Mapping[str, Any], summary: Mapping[str, Any]) -> str:
    ...
    comments = [
        "# Generated by scripts/build_production_configs.py; do not copy bucket sizes from the sample fixture.",
